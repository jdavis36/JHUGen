...
\author{I. Anderson, S. Bolognesi, F. Caola, J. Davis, Y. Gao, A. V. Gritsan, L. S. Mandacaru Guerra, Z. Guo, Z. Huang, L. Kang, S. Kyriacou, C. B. Martin, T. Martini, K. Melnikov, 
...
(version v7.6.0, release date June 12, 2026) \\
\normalsize
\end{center}
...
also depend on MCFM libraries for background parameterization which should be referenced~\cite{Campbell:2010ff} when used. Special thanks to Ulrich Haisch and Gabriël Koole for their efforts in including
the Higgs self-coupling~\cite{SelfCoupling}, and to Oscar Eboli for introducing anomalous Zqq couplings, into the gluon fusion process for MCFM.
Certain processes, such as quark-antiquark Higgs production, similarly depend on Madgraph utilizing SMEFTsim~\cite{SMEFTsim1,SMEFTsim2,Madgraph},
colloquially referred to as ``madMELA''.
In order to simplify the translation between different coupling conventions, we provide the JHUGenLexicon program within the JHU generator framework,
...
\begin{verbatim}
  ...
  Process configuration:
    ...
    Process:           0=spin-0, 1=spin-1, 2=spin-2 resonance,
                       10=phase space in decay of a resonance,
                       ...
                       117=t/tbar+W+H
    Polarization:      For Process 0:
                       fL = longitudinal polarization (-1, 1),
                       fPerp = CP-odd transverse polarization (-1, 1) 
                       calc_fL = method for modelling polarized states (1 to activate)
                       For Process 2:
                       calc_fAmp = option for Jz^P of resonance 
                       (-1 for Jz^P = 1^-, 1 for 1^+,2 for 2^+, -11 for interference 1^+ and 1^-)
...
The couplings used by the Madgraph interface utilize the SMEFTsim package for Madgraph \cite{SMEFTsim1, SMEFTsim2}. These are expressed as real-valued Wilson Coefficients, and affect far more than just the amplitudes within couplings.
...
The Python framework also interfaces with couplings differently, as they are named couplings. For instance, while in C\texttt{++} couplings are set via arrays, in Python they can \textit{also} be set via named values. For instance, the Pythonic version of the code snippet for Self-Defined couplings above is depicted below:
\begin{verbatim}
  import Mela
  m = Mela.Mela()
  m.setProcess(
...
\begin{verbatim}
    ...
    data clanot / 0d0 / shift in left handed coupling of Z to top quarks in production
    data cranot / 0d0 / shift in right handed coupling of Z to top quarks in production
\end{verbatim}
...
mela->selfDAZffcoupl[gAZff_tZRH] = 0.2
\end{verbatim}

This would correspond to SM Zll couplings, and SM Z couplings to right handed quarks. The Z coupling to right-handed neutrinos is set to 1, SM Z coupling to left-handed
up-type quarks in production is shifted by 0.1 and SM Z coupling to right-handed top quarks is shifted by 0.2 . Again in this example, since the other Zff couplings are not set, they are set to 0 in the matrix element calculation. Please also note that applying a shift to the up-type quarks and the top quark will result in the SM Z coupling to the top quark being shifted twice.

\section{Release notes}
\noindent
In going from \verb|v7.5.7| to \verb|v7.6.0| (June 12, 2026) the updates are as follows:
\begin{itemize}
    \item Introduced Higgs self-coupling in EW production into the MCFM matrix element.
    \item Added \verb|Process=10| which allows for uniformly weighted simulation of $gg \to X\to VV$ phase space without consideration of the matrix-element
    \item Introduce polarization framework, which allows for simulation of specific polarization fractions in the decay of any resonance $X\to VV$
    \item Separated the anomalous couplings of the Z boson fermion couplings in the MCFM matrix element to privilege the top quark from the other up type quarks.
    \item Add new option to JHUGenLexicon for applying the MCFM convention to anomalous triple and quartic-gauge couplings 
    \item Add width scheme 5 for running widths of very narrow resonances
\end{itemize}
\noindent
In going from \verb|v7.5.6| to \verb|v7.5.7| (December 20, 2024) the updates are as follows:
\noindent
\begin{itemize}
\item Introduce $q^2$ dependent form factor in Higgs boson propagator in MCFM matrix elements. 
\item Introduced signal, background, and BSI matrix elements for $\{u,d,c,s,b\} \rightarrow 4\ell$ utilizing Madgraph matrix elements within madMELA.
\item Introduced Higgs self-coupling for gluon-fusion into the MCFM matrix element.
We would like to thank Ulrich Haisch and Gabriël Koole for their efforts in including
the Higgs self-coupling into the gluon fusion process for MCFM~\cite{SelfCoupling}.
\item Corrected a sign convention in one term within JHUGenLexicon
\end{itemize}
\noindent
...
\bibitem{SelfCoupling}
Haisch, U., Koole, G., ``{Off-shell Higgs production at the LHC as a probe of the trilinear Higgs coupling}'',
{ J. High Energ. Phys. } (2022) 30,
doi{10.48550}, arXiv:2111.12589v2 [hep-ph]
\bibitem{Madgraph}
J.~Alwall {et-al}, ``{Madgraph 5: Going Beyond}'',
JHEP 06 (2011) 128, https://arxiv.org/abs/1106.0522, arXiv:1106.0522
\end{thebibliography}
\end{document}